...
class UNet3D4LAC(nn.Module):
    def __init__(self, in_channels=2, out_channels=1, base_features=32):
        ...
        self.enc2.conv = TripleConv(base_features, base_features * 2)
        ...
        # Decoder (Attention + TripleConv)
        self.att4 = AttentionBlock(g_channels=base_features*16, x_channels=base_features*8, inter_channels=base_features*8)
        ...
        self.att3 = AttentionBlock(g_channels=base_features*8, x_channels=base_features*4, inter_channels=base_features*4)
        ...
        self.att2 = AttentionBlock(g_channels=base_features*4, x_channels=base_features*2, inter_channels=base_features*2)
        ...
        self.att1 = AttentionBlock(g_channels=base_features*2, x_channels=base_features, inter_channels=base_features)
    ...
    def forward(self, x):
        ...
        # Decoder: skip bağlantılarına attention uygula
        att4 = self.att4(bottleneck, enc4)
        dec1 = self.dec1(bottleneck, att4)

        att3 = self.att3(dec1, enc3)
        dec2 = self.dec2(dec1, att3)

        att2 = self.att2(dec2, enc2)
        dec3 = self.dec3(dec2, att2)

        att1 = self.att1(dec3, enc1)
        dec4 = self.dec4(dec3, att1)
