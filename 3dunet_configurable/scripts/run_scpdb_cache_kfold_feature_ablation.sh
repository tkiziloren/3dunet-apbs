...
OUTPUT_ROOT="${OUTPUT_ROOT:-/Users/tevfik/Sandbox/github/PHD/runs/scpdb_cache_kfold_feature_ablation_${EPOCHS}epoch_thr${VALIDATION_THRESHOLD/./}}"
...
echo "K-fold feature ablation output root: $OUTPUT_ROOT"
...
export BASE_CONFIG CONFIG_DIR EPOCHS EARLY_STOPPING_PATIENCE VALIDATION_THRESHOLD RUN_FILTER SPLIT_DIR FOLDS
"$PYTHON_BIN" - <<'PY'
import copy
import csv
import os
...
folds = [
    int(item.strip())
    for item in os.environ["FOLDS"].split(",")
    if item.strip()
]
run_filter = {
    item.strip()
...
for global_run_index, (fold_idx, suffix, features) in enumerate(requested_runs, start=1):
    ...
    config["name"] = f"scpdb_cache_kfold_ablation_fold{fold_idx}_{suffix}"
    config["feature_set"] = {
...
echo
echo "============================================================"
echo "Summarizing k-fold feature ablation runs"
echo "============================================================"
